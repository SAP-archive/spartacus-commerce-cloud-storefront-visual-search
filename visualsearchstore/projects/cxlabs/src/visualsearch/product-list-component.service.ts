 ...
 * TODO: This class can be removed after Spartacus 2.1
 */
 ...
 */
@Injectable()
export class VisualSearchProductListComponentService {
  /**
  ...
  constructor(
    ...
    protected visualService: VisualSearchService
  ) {}
  ...
  protected search(criteria: SearchCriteria): void {
    ...
    if (criteria.query && criteria.query.startsWith('visual-')) {
      this.visualSearch(criteria);
    } else {
  ...
  /**
   * Performs a visual search based on the uploaded image and selected bounding box.
   * It calls microservice first to collect similar product ids,
   * transforms search query and sortCode and delegates to `ProductSearchService`.
   */
  protected visualSearch(criteria: SearchCriteria) {
    const currentPage = criteria.currentPage;
    const pageSize = criteria.pageSize;

    const id = criteria.query.split('visual-')[1];
    this.visualService.getSimilarProducts(id).subscribe((res) => {
      const sortCode = 'visual-relevance';
